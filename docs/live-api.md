...
Discogs accepts unauthenticated requests but applies a stricter rate limit (25 req/min vs 60 req/min with a token). Get a personal access token from [discogs.com/settings/developers](https://www.discogs.com/settings/developers) and export it:

```bash
export DISCOGS_TOKEN=your_token_here
```

All functions accept an optional `token=` keyword argument, if you prefer not to use an environment variable.
...
All four functions return the same model classes the bulk-dump path uses (`Artist`, `Release`, `Master`, `Label`), so downstream code works with either source interchangeably.
...
`search()` returns the raw API dict with `"results"` and `"pagination"` keys. `search_iter()` takes the same arguments plus `limit=` and yields individual result dicts across all pages.

## Rate limiting

- Without a token: 25 req/min.
- With a token: 60 req/min.
- The client enforces a roughly 1.1 s inter-request delay by default (about 54 req/min).
- On HTTP 429, one automatic retry after 5 s.
- Set `PYDISCOGS_ANON=1` to route through rotating proxies (requires `anon_requests`) for sustained high-volume use.

## Verified live

Development called the following endpoints directly (no token needed for individual-entity GETs):
...
- `GET /database/search?q=Stockholm&type=release&per_page=2` returns paginated results including release 1.

Responses are captured as JSON fixtures in `tests/fixtures/live_*.json` and used by the offline test suite.
